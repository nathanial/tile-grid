...
export default class InteractiveTest extends React.Component<{}, {}>{
	...
	_renderTiles(){
		...
		for(let tileKey of _.keys(this.state.tiles)){
			...
			if(value){
				tiles.push(
					<Tile key={tileKey} column={parseInt(tileX)} row={parseInt(tileY)} style={{background: 'blue', color: 'white'}}>
					</Tile>
				);
			}
		}
